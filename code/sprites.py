...
class Bee(Enemy):
  def __init__(self, frames, pos, groups, speed, difficulty_multiplier):
    super().__init__(frames, pos, groups)
    self.speed = speed
    self.amplitude = randint(500, 600)
    self.frequency = randint(300, 600)
    self.speed = speed * difficulty_multiplier  # Scale based on difficulty
  ...
  def move(self, dt):
    self.rect.x -= self.speed * dt
    self.rect.y += sin(
      pygame.time.get_ticks() / self.frequency) * self.amplitude * dt
...
class Worm(Enemy):
  ...
  def constraint(self):
    if not self.main_rect.contains(self.rect):
      self.direction *= -1
      self.frames = [pygame.transform.flip(surf, True, False) for surf in
                     self.frames]
...
class Player(AnimatedSprite):
  ...
  def check_floor(self):
    bottom_rect = pygame.FRect((0, 0), (self.rect.width, 2)).move_to(
